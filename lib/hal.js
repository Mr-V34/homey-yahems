...
function validateMap(rawMapOrString) {
  ...
  if (rawCustom.length > MAX_CUSTOM) {
    ...
    for (let i = 0; i < rawCustom.length; i++) {
      ...
      // A custom consumer is "name + mode" — a mapped device is OPTIONAL. The status
      // insight (allowed/paused) is derived from DEFCON + mode and needs no device.
      // If a device IS mapped, a capability must come with it (kept for future power read).
      const hasDevice = typeof c.deviceId === 'string' && c.deviceId;
      if (hasDevice && (typeof c.capability !== 'string' || !c.capability)) {
        errors.push(`${p}: capability required when a device is mapped`); continue;
      }
      ...
      if (hasDevice) { clean.deviceId = c.deviceId; clean.capability = c.capability; }
...
function selfTest() {
  ...
  const vCustom = validateMap({
    ...
    custom: [
      ...
      { name: 'Bordslampa', mode: 'pausable' }, // name + mode, no device → ACCEPTED (device optional)
      { deviceId: 'dev-x' }, // device but no capability → rejected
    ],
  });
  chk(vCustom.ok === false, 'custom with a device but no capability → ok:false');
  chk(vCustom.map.custom.length === 3, `3 valid custom kept (incl. device-less), got ${vCustom.map.custom.length}`);
  ...
  chk(vCustom.map.custom[2].deviceId === undefined && vCustom.map.custom[2].name === 'Bordslampa',
    `device-less custom kept without deviceId, got ${JSON.stringify(vCustom.map.custom[2])}`);
