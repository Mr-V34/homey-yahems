...
function decideDevices(input, matrix = DEFAULT_MATRIX, boosts = {}) {
  ...
  return {
    ...
    // Each white-goods device decides independently. The legacy group boost key
    // 'appliances' boosts all three; per-device boost keys also work.
    dishwasher: decideAppliance(s, matrix, 'dishwasher', !!(b.appliances || b.dishwasher)),
    washer: decideAppliance(s, matrix, 'washer', !!(b.appliances || b.washer)),
    dryer: decideAppliance(s, matrix, 'dryer', !!(b.appliances || b.dryer)),
...
function selfTest() {
  ...
  const plLow = decideDevices({
    defcon: 5, consumptionW: 0, priceLevel: 0,
    ev: { connected: true, mustCharge: true },
  });
  ...
  const poCeiling = decideDevices({
    defcon: 5, consumptionW: 0, priceLevel: 3, priceOre: 99999,
    ev: { connected: true, mustCharge: true },
  });
