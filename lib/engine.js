...
/** Effective anchor with optional night-tariff multiplier window (local hour). */
function effectiveAnchor(anchorW, floorW, opts = {}) {
  const base = Math.max(floorW || 0, anchorW);
  const { nightTariff = false, localHour = 0, nightStart = 22, nightEnd = 6, nightMult = 2 } = opts;
  const isNight = nightTariff
    && (nightStart > nightEnd
      ? (localHour >= nightStart || localHour < nightEnd)
      : (localHour >= nightStart && localHour < nightEnd));
  return Math.round(base * (isNight ? nightMult : 1));
}
...
module.exports = { clamp, defconFromNet, rollingAverage, effectiveAnchor, selfTest };
